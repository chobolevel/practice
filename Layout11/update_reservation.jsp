...
<%
	...
	ResultSet rs = null;
	String sql = "select custname, bookno, to_char(outdate, 'YYYY-MM-DD'), to_char(indate, 'YYYY-MM-DD'), status, class from reservation0531 where lentno = ?";
	if(custname == null) {
		try {
...
<%
	...
	pstmt = null;
	rs = null;
	sql = "select bookname from bookinfo0531 where bookno = ?";
	try {
		pstmt = conn.prepareStatement(sql);
...
%>
<h2>도서 대출 예약 정보 수정 화면</h2>
<form name = "form" method = "post" action = "update_reservation.jsp">
...
<tr>
	<th>대출상태</th>
	<td>
		<input type = "radio" name = "status" value = "1" <%if(indate == null || indate == "") out.print("checked"); %>>대출
...
function check() {
	if(!document.form.lentno.value || document.form.lentno.value == "") {
		...
		form.action = "update_reservationProcess.jsp";
